        ...
        while(true){
           /* System.out.println("++++++++++经过第"+num+"次分区后++++++++++");
            ...
            for(List<Point> list :region){
                ...
                double[][] BBMatrix=dlist.dMatrix();
                Node node = new BBTSP(BBMatrix,list_bk).solve();
                System.out.println("最小路径为:"+Double.parseDouble(df.format(node.lb))+"m");
                System.out.println("构成的边为：");
                StringBuilder sb=new StringBuilder(list_bk.get(0).getAddress());
                for(int i=0,j=0;i<list_bk.size()&&j<list_bk.size();j++){
                    int value=node.map_edge.get(i);
                    sb.append(" -> "+list_bk.get(value).getAddress());
                    i=value;
                }
                ...
                double t=(node.lb/20)/1000+2.0/3.0;
